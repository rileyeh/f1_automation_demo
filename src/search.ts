...
export async function search(driver: WebDriver, searchTerm: string) {
    ...
    await (await driver.findElement(By.name('q'))).clear()
    
    await driver.findElement(By.name('q')).sendKeys('Batman\n')

    let images = await driver.findElement(By.xpath('(//a[text()="Images"])[1]'))
